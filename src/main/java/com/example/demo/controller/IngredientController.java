...
public class IngredientController {
	...
	public IngredientBean addIngredient(@RequestPart String ingredientJson,
		...
		// 是一个用於將 JSON 字串轉換為 Java 對象的操作。這個操作涉及到 Jackson 庫的 ObjectMapper 類。
		// ObjectMapper 是 Jackson 庫中的一個核心類，用於處理 JSON 數據。它提供了將 Java 對象與 JSON 數據相互轉換的功能。
		// ObjectMapper 可以將 JSON 字符串解析為 Java 对象，也可以將 Java 對象序列化為 JSON 字符串。
		
		// readValue 方法是 ObjectMapper 類中的一個方法，它用於將 JSON 字符串轉換為 Java 對象。
		...
		// valueType：將 JSON 轉換成的目標 Java 類的类型。
